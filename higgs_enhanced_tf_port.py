...
from tensorflow.keras.layers import Input,Dense, Dropout, Activation, SpatialDropout1D
...
def GetInputDim(features = 'raw'):
  """
  The original paper alteratively trained on the first 22 raw features,
...
def main():
  ...
  for _ in range(n_layers):
    # add layer
    AddLayer(model)
    model.fit(X_train, y_train, epochs = 1, batch_size = args.batch_size, validation_split = args.ttsplit)
